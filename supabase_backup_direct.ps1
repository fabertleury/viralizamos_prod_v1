# Script PowerShell para backup direto do Supabase usando pg_dump
# Autor: Cascade AI
# Data: 03/03/2025
...
try {
    ...
    & $pgDumpPath -h $host -p $port -U $user -d $dbName -f $backupFile
    
    if (Test-Path $backupFile) {
        $fileSize = (Get-Item $backupFile).Length / 1MB
        Write-Host "Backup concluído: $backupFile (Tamanho: $($fileSize.ToString('0.00')) MB)"
